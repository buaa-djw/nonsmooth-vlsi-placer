...
int main(int argc, char **argv)
{
    ...
    try
    {
        ...
        if (!positive_row_heights.empty())
        {
            ...
            row_height = n % 2 ? positive_row_heights[n / 2] : 0.5 * (positive_row_heights[n / 2 - 1] + positive_row_heights[n / 2]);
        }
        ...
        if (cfg.quadratic_init)
        {
            std::cout << "[quadratic] L" << coarsest.index 
                      ...
                      << " iterations=" << cfg.quadratic_iterations 
                      << std::endl;

            quadratic_result = placer::quadraticInitialize(coarsest, region, {cfg.quadratic_iterations, cfg.quadratic_damping, cfg.quadratic_anchor, cfg.quadratic_tolerance, cfg.seed});
        }
        ...
        for (int li = (int)levels.size() - 1; li >= 0; --li)
        {
            ...
            if (li + 1 < (int)levels.size())
            {
                ...
                if (std::abs(consistency.relative_delta) > cfg.hpwl_continuity_tol)
                    std::cout << "[warning] inter-level HPWL relative delta exceeds " << cfg.hpwl_continuity_tol << std::endl;
            ...
            << " movable=" << lev.movableIds().size() 
            << " bins=" << bx 
            << "x" << by 
            << std::endl;

            auto res =  placer::optimizeLevel(
                level,
                region,
                density_grid,
                optimize_config,
