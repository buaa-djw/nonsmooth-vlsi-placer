...
namespace placer
{
    namespace
    {
        ...
        std::vector<std::pair<int, int>> ringCandidates(int ring)
        {
    ...
    {
        MacroShiftStats st;
        std::vector<double> macro_sizes;
        for (const auto &o : l.objects)
            if (!o.fixed && o.is_macro)
                macro_sizes.push_back(std::max(o.width, o.height));
        const double cell_size = std::max(row_height, medianOr(macro_sizes, row_height));
        SpatialHash h(cell_size);
        ...
        st.macros = ids.size();
        const double step = std::max(row_height, std::min((r.xh - r.xl) / 100.0, (r.yh - r.yl) / 100.0));
        for (auto id : ids)
        {
            auto &o = l.objects[id];
            const double ox = o.x, oy = o.y;
            std::optional<Candidate> best;
            for (int ring = 0; ring <= rings; ++ring)
            {
                best.reset();
                for (auto [dx, dy] : ringCandidates(ring))
                {
                    const double x = std::min(std::max(ox + static_cast<double>(dx) * step, r.xl), std::max(r.xl, r.xh - o.width));
                    const double y = std::min(std::max(oy + static_cast<double>(dy) * step, r.yl), std::max(r.yl, r.yh - o.height));
                    Rect rr{x, y, x + o.width, y + o.height, id};
                    if (h.collides(rr, gap))
                        continue;
                    const double score = (x - ox) * (x - ox) + (y - oy) * (y - oy);
                    if (!best || score < best->score)
                        best = Candidate{score, x, y};
                }
                if (best)
                    break;
            }
