...
namespace placer
{
    namespace
    {
        ...
        std::unique_ptr<SliceNode> buildSliceTree(const Level &level, std::vector<size_t> ids, const Region &bounds, int leaf_size)
        {
            ...
            std::stable_sort(ids.begin(), ids.end(), [&](size_t a,size_t b){ double ka=split_x?level.objects[a].cx():level.objects[a].cy(), kb=split_x?level.objects[b].cx():level.objects[b].cy(); return ka<kb; });
            ...
            if(split_x){ double cut=0.5*(level.objects[ids[mid-1]].cx()+level.objects[ids[mid]].cx()); cut=std::min(bounds.xh-EPS,std::max(bounds.xl+EPS,cut)); node->old_cut=cut; node->left=buildSliceTree(level,l,{bounds.xl,cut,bounds.yl,bounds.yh},leaf_size); node->right=buildSliceTree(level,r,{cut,bounds.xh,bounds.yl,bounds.yh},leaf_size); }
            else { double cut=0.5*(level.objects[ids[mid-1]].cy()+level.objects[ids[mid]].cy()); cut=std::min(bounds.yh-EPS,std::max(bounds.yl+EPS,cut)); node->old_cut=cut; node->left=buildSliceTree(level,l,{bounds.xl,bounds.xh,bounds.yl,cut},leaf_size); node->right=buildSliceTree(level,r,{bounds.xl,bounds.xh,cut,bounds.yh},leaf_size); }
            return node;
        }
        double computeDemand(SliceNode &node, const Level &level, const std::vector<Region> &fixed_rects, double target_density)
        {
            if(!node.left || !node.right){ double movable_area=0; for(auto id:node.ids) movable_area+=level.objects[id].area(); movable_area/=std::max(target_density,EPS); double fixed_area=0; for(const auto &r:fixed_rects) fixed_area+=rectOverlapArea(node.bounds.xl,node.bounds.xh,node.bounds.yl,node.bounds.yh,r.xl,r.xh,r.yl,r.yh); node.demand=movable_area+fixed_area; return node.demand; }
            node.demand=computeDemand(*node.left,level,fixed_rects,target_density)+computeDemand(*node.right,level,fixed_rects,target_density); return node.demand;
        }
        ...
        {
            if(!node.left || !node.right || !node.axis){ for(auto id:node.ids){ auto &o=level.objects[id]; o.setCenter(affineMap(o.cx(),node.bounds.xl,node.bounds.xh,nb.xl,nb.xh), affineMap(o.cy(),node.bounds.yl,node.bounds.yh,nb.yl,nb.yh)); } return; }
