...
ProjectionStats projectObject(LObject &o, const Region &r) {
  ...
  double left = -0.5 * o.width, right = 0.5 * o.width, bottom = -0.5 * o.height,
         top = 0.5 * o.height;
  ...
  if (lo_cx > hi_cx + EPS || lo_cy > hi_cy + EPS)
    throw std::runtime_error(
        "object/envelope does not fit in placement region: " + o.name);
  o.setCenter(std::min(std::max(o.cx(), lo_cx), hi_cx),
              std::min(std::max(o.cy(), lo_cy), hi_cy));
...
ProjectionStats projectLevel(Level &l, const Region &r) {
  ...
  for (auto &o : l.objects) {
    auto s = projectObject(o, r);
    total.projected_objects += s.projected_objects;
    total.max_shift = std::max(total.max_shift, s.max_shift);
  }
  return total;
}
...
void seedGrid(Level &l, const Region &r, int seed) {
  ...
  const int ny =
      std::max(1, static_cast<int>(std::ceil(static_cast<double>(count) /
  ...
  for (std::size_t k = 0; k < order.size(); ++k) {
    ...
    const double cx = r.xl + (static_cast<double>(ix) + 0.5) * (r.xh - r.xl) /
                                 static_cast<double>(nx);
    const double cy = r.yl + (static_cast<double>(iy) + 0.5) * (r.yh - r.yl) /
