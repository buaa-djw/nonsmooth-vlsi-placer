...
namespace placer
{
    DeclusterStats decluster(Level &f, Level &c, const Region &r)
    {
        ...
        {
            size_t pid = (*c.fine_to_coarse)[cid];
            if (pid >= c.objects.size())
                throw std::runtime_error("invalid fine_to_coarse for decluster");
        ...
        for (size_t pid = 0; pid < c.objects.size(); ++pid)
        {
            ...
            double lo = ax, hi = ax, loy = ay, hiy = ay;
            bool any = false;
            for (auto cid : children[pid])
                if (!f.objects[cid].fixed)
                {
                    ...
                    lo = any ? std::max(lo, r.xl + 0.5 * ch.width - dx) : r.xl + 0.5 * ch.width - dx;
                    hi = any ? std::min(hi, r.xh - 0.5 * ch.width - dx) : r.xh - 0.5 * ch.width - dx;
                    loy = any ? std::max(loy, r.yl + 0.5 * ch.height - dy) : r.yl + 0.5 * ch.height - dy;
                    hiy = any ? std::min(hiy, r.yh - 0.5 * ch.height - dy) : r.yh - 0.5 * ch.height - dy;
                    any = true;
                }
            if (any)
            {
                if (lo > hi + EPS || loy > hiy + EPS)
                {
                    ++s.impossible_groups;
                    throw std::runtime_error("decluster child group cannot fit region");
                }
                ax = std::min(std::max(ax, lo), hi);
                ay = std::min(std::max(ay, loy), hiy);
            }
