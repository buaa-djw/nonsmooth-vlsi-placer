...
namespace placer
{
    namespace
    {
        ...
        double l2(const std::vector<double> &x, const std::vector<double> &y, const std::vector<size_t> &ids)
        {
            double v = 0;
            for (auto i : ids)
                v += x[i] * x[i] + y[i] * y[i];
        ...
        double dot(const std::vector<double> &ax, const std::vector<double> &ay, const std::vector<double> &bx, const std::vector<double> &by, const std::vector<size_t> &ids)
        {
            double v = 0;
            for (auto i : ids)
        ...
        void capture(const Level &l, const std::vector<size_t> &ids, std::vector<double> &x, std::vector<double> &y)
        {
            x.resize(ids.size());
            y.resize(ids.size());
        ...
        void restore(Level &l, const std::vector<size_t> &ids, const std::vector<double> &x, const std::vector<double> &y)
        {
            for (size_t k = 0; k < ids.size(); ++k)
            {
    ...
    double paperStepScale(int iteration) { 
        return std::max(0.2 * std::pow(2.0 / 3.0, static_cast<double>(iteration / 100)), 0.06); 
    }

    double paperInitialLambda(double w, double d)
    {
        if (!std::isfinite(w) || !std::isfinite(d) || d <= 0.0)
            throw std::runtime_error("initial layout has no valid density gradient (density gradient L1 is zero)");
        double v = w / d;
        if (!std::isfinite(v))
            throw std::runtime_error("non-finite initial lambda");
        return v;
    }
    double polakRibiereBeta(const std::vector<double> &g, const std::vector<double> &p)
    {
        if (g.size() != p.size())
    ...
    double nextPaperLambda(double lambda, double current, double previous, bool has_previous)
    {
        double f = current < 0.04 ? 1.6 : (has_previous && current < 0.5 * previous ? 1.9 : 2.2);
        return lambda * f;
    }

    int paperNoImprovementLimit(size_t n) { 
        return std::max(1, std::min(static_cast<int>(std::ceil(0.001 * static_cast<double>(n))), 100)); 
    }
    ...
    OptimizeResult optimizeLevel(Level &l, const Region &r, const DensityGrid &grid, const OptimizeConfig &cfg, GlobalOptimizeState &gs)
    {
        ...
        {
            if (stage > 0)
                out.lambda = nextPaperLambda(out.lambda, accepted_ofr, stage > 1 ? out.stages[out.stages.size() - 2].ofr : 0.0, stage > 1);
            ...
            for (int it = 0; it < cfg.iterations_per_stage; ++it)
            {
                ...
                row.overflow_bin_count = d.overflow_bins_report;
                row.total_overflow = d.ofr_report;
                ...
                row.elapsed_sec = std::chrono::duration<double>(std::chrono::steady_clock::now() - gs.start_time).count();
                out.history.push_back(row);
                sr.iterations = it + 1;
                if (cfg.report_every > 0 && row.global_iteration % cfg.report_every == 0)
                    std::cout << "[L" << cfg.level_index 
                              << " S" << stage 
                              << " I" << it 
                              << "] HPWL=" << w.hpwl 
                              << " P=" << d.penalty 
            ...
            if (stage > 0 && !improved(accepted_ofr, sr.ofr))
            {
                ...
                out.stop_reason = "ofr_not_improved";
                break;
            }
            accepted_ofr = sr.ofr;
            capture(l, ids, accepted_x, accepted_y);
            have_accepted = true;
            if (sr.ofr <= cfg.target_ofr)
                break;
        }
        if (have_accepted)
            restore(l, ids, accepted_x, accepted_y);
